...
import { categories, experienceLevels } from '../data/mockData';
...
const handleSearch = async () => {
  ...
  try {
    const res = await fetch(
      `http://localhost:5000/search?q=${encodeURIComponent(draftSearchQuery)}`,
      {
        headers: {
    ...
    const filtered = mapped.filter((user: any) => {
      return (
        (!draftCategory || user.skills.some((s: any) => s.category === draftCategory)) &&
        (!draftSubCategory || user.skills.some((s: any) => s.subCategory === draftSubCategory)) &&
        (!draftLevel || user.skills.some((s: any) => s.experienceLevel === draftLevel))
      );
    });
