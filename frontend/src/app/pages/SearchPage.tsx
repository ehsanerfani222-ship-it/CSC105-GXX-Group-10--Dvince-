...
const handleSearch = async () => {
  ...
  try {
    ...
    // ✅ apply frontend filters (keep UX behavior)
    const filtered = data.filter((user: any) => {
      return (
        (!draftCategory ||
          user.skills.some((s: any) => s.category === draftCategory)) &&
        (!draftSubCategory ||
          user.skills.some((s: any) => s.subCategory === draftSubCategory)) &&
        (!draftLevel ||
          user.skills.some((s: any) => s.experienceLevel === draftLevel))
      );
    });
