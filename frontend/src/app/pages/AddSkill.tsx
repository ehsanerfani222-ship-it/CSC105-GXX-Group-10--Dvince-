...
// Days that can be selected in the schedule section.
const daysOfWeek = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'];

// Creates one empty skill object for the form.
...
// Shape of the optional router state that controls navigation behavior.
...
export default function AddSkill() {
  ...
  // Read the current user's already saved skills.
  const existingSkills = getCurrentUserProfile().skills;

  // Read the optional navigation state passed from the previous page.
  const navigationState = (location.state as AddSkillLocationState | null) ?? null;

  // Decide whether the page is currently used for editing or for onboarding/adding.
  const isEditMode = navigationState?.mode === 'edit';

  // Use dynamic fallback paths:
  // - edit flow: go back to /my-skills and also finish at /my-skills
  // - onboarding flow: go back to /create-profile and finish at /search
  ...
  // Pre-fill the form with existing skills if available.
  // Otherwise start with one empty skill form.
  const [skills, setSkills] = useState<Skill[]>(
    existingSkills.length > 0 ? existingSkills : [createEmptySkill(1)]
  );

  // Adds another empty skill card to the form.
  const addNewSkill = () => {
    setSkills((current) => [...current, createEmptySkill(Date.now())]);
  };

  // Removes a skill card, but keeps at least one form visible.
  ...
  // Updates one top-level field of a skill.
  ...
  // Updates one field inside the schedule object.
  ...
  // Toggles a single day in the schedule.
  ...
  // Reads uploaded images/videos and stores them as base64 strings in the skill.
  ...
  // Saves all skills and then navigates to the correct next page
  // depending on whether the user is adding or editing skills.
  const handleComplete = () => {
    saveUserSkills(skills);
    navigate(nextPath);
  };

  // Goes back to the page that opened this screen.
  const handleBack = () => {
    navigate(backPath);
  };
