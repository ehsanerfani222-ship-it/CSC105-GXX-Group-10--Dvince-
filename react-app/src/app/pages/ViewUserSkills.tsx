...
export default function ViewUserSkills() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20 lg:pb-0">
      <Header showBackButton onBackClick={() => navigate(`/user/${user.id}`)} />

      <main className="container mx-auto px-4 md:px-6 lg:px-8 py-8">
        <div className="max-w-4xl mx-auto">
          <div className="flex items-end justify-between gap-4 mb-6">
            <div>
              <h2 className="text-2xl md:text-3xl font-bold text-gray-900">{user.fullName}'s Skills</h2>
            ...
            {user.skills.map((skill) => (
              <div key={skill.id} className="bg-white rounded-2xl shadow-lg p-6 md:p-8">
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  <span className="bg-cyan-100 text-cyan-700 px-3 py-1 rounded-full text-sm font-medium">{skill.category}</span>
                  <span className="bg-gray-100 text-gray-700 px-3 py-1 rounded-full text-sm font-medium">{skill.subCategory}</span>
                  <span className="bg-green-100 text-green-700 px-3 py-1 rounded-full text-sm font-medium">{skill.experienceLevel}</span>
                </div>
